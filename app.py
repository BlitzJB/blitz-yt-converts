...
def show_song_details(song):
    ...
    with col2:
        ...
        st.markdown(f"**Duration:** {format_duration(song.get('duration_seconds', 0) * 1000)}")
...
if st.session_state.selected_song:
    ...
    with col2:
        ...
        if not st.session_state.download_complete:
            if st.button("Start Download", use_container_width=True, key="download_btn"):
        ...
        if st.button(" Back to Search", use_container_width=True, key="back_btn", type="secondary"):
            st.session_state.selected_song = None
            st.session_state.download_complete = False
            st.session_state.download_data = None
            st.rerun()
                ...
                for idx, song in enumerate(results):
                    # if song['resultType'] == 'song':
                        ...
                        with col2:
                            ...
                            if st.button("Select", key=f"select_{idx}_{song.get('videoId', '')}", type="secondary", use_container_width=True):
